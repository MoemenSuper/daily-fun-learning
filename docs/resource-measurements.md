...
The first umbrella Windows App SDK build measured 154.38 MiB because it included unused WinUI, DirectML, ONNX, widgets, and AI components. Referencing only the official Foundation and Runtime packages reduced the published footprint to 22.60 MiB while preserving `Microsoft.Windows.AppNotifications.AppNotificationManager`.
