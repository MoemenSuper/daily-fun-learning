...
'In `run(sayHi)`, JavaScript first evaluates the argument expression `sayHi`. That name comes from `const sayHi = ...`, and its value is a function object. JavaScript then creates the local parameter `task` for this call and stores a reference to the same function object in it. The names do not need to match: `task` is a new local name for the value that arrived. Inside `run`, the parentheses in `task()` call that function, which returns `"hi"`. If you wrote `run(sayHi())`, JavaScript would call `sayHi` first and pass the returned string instead. Calling that string as `task()` would throw a `TypeError`. Trace the code in four steps: argument expression, resulting value, parameter name, and use inside the function.',
'function run(task) { return task(); }\nconst sayHi = () => "hi";\nrun(sayHi);',
